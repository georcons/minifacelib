"""CNN-backed face predictor implementation."""
...
from minifacelib.models.base import FacePrediction, FacePredictor, parse_emotion, parse_gender
...
class CNNFacePredictor(FacePredictor):
    """Face predictor that runs local CNN models for emotion, gender, and age."""

    MODEL_NAME: ClassVar[str] = "cnn"
    _IMAGE_SIZE = 64

    def __init__(self, device: str | None = None):
        """Load model weights and initialize inference models."""
        ...
        self._gender_labels: dict[int, str] = {
            int(k): str(v) for k, v in (gender_age_pt.get("gender_labels", {}) or {}).items()
        }
    ...
    def predict(self, image: Image.Image) -> FacePrediction:
        """Run emotion, gender, and age inference on a single image."""
